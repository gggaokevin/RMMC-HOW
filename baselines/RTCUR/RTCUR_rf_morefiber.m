% rf - resampled fiber
function [L_core,X_sub_mat,timer,err ] = RTCUR_rf_morefiber(D, R, para)
n_dim = size(D);
...
for i_mod = 1:n
    ...
    len2(i_mod) = min(len1(i_mod)^2,prod(n_dim)/d_i);
...
for i_mod = 1:n
    I{i_mod} = randperm(n_dim(i_mod),len1(i_mod));
    J{i_mod} = randperm(prod(n_dim)/n_dim(i_mod),len2(i_mod));
end
